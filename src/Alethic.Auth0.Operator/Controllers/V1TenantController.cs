        ...
        IEntityController<V1Tenant>
    {
        ...
        protected override async Task Reconcile(V1Tenant entity, CancellationToken cancellationToken)
        {
            ...
            if (needsAuth0Fetch)
            {
                ...
                LogAuth0ApiCall($"Getting Auth0 tenant settings", "read", "A0Tenant", entity.Name(), entity.Namespace());
            ...
            if (entity.Spec.Conf is { } conf && needsAuth0Fetch)
            {
                ...
                if (needsUpdate)
                {
                    ...
                    try
                    {
                        ...
                        LogAuth0ApiCall($"Updating Auth0 tenant settings", "write", "A0Tenant", entity.Name(), entity.Namespace());
            ...
            if (needsAuth0Fetch)
            {
                ...
                LogAuth0ApiCall($"Getting Auth0 tenant settings for status update", "read", "A0Tenant", entity.Name(), entity.Namespace());
                settings = await api.TenantSettings.GetAsync(cancellationToken: cancellationToken);
